...
class Alumno{
    ...
    public function actualizar_alumno($conn){
        $consulta="UPDATE alumno
        SET nombre=:nombre, apellido=:apellido, email=:email, nacimiento=:nacimiento
        WHERE DNI=:DNI";
    ...
    public function checkear_actualizacion($conn){
        $consulta="SELECT *
        FROM alumno
        WHERE DNI=:DNI and nombre=:nombre and apellido=:apellido and email=:email and nacimiento=:nacimiento";
    ...
    public static function buscar_cumpleanios($conn, $codigo_materia){
        $consulta="SELECT *
        FROM alumno
        INNER JOIN materia_alumno ON alumno.DNI = materia_alumno.DNI
        WHERE DATE_FORMAT(nacimiento,'%m,%d')=DATE_FORMAT(CURDATE(),'%m,%d') and materia_alumno.codigo_materia = :codigo_materia";

        $stmt=$conn->prepare($consulta);
        $stmt->bindparam(":codigo_materia",$codigo_materia);
        $stmt->execute();
        $cumpleanios=$stmt->fetchAll(PDO::FETCH_ASSOC);
